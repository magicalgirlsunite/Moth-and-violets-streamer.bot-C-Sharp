...
public class CPHInline
{
    public bool Execute()
    {
        ...
        // 4. THE SIMULTANEOUS CLIP & VIDEO PLAYER (STREAMLABS VERSION)
        ...
        if (!string.IsNullOrEmpty(clipId))
        {
            ...
            CPH.SlobsSetBrowserSource(sceneName, clipBrowserSource, embedUrl);
            
            // Turn the clip player ON
            CPH.SlobsSetSourceVisibility(sceneName, clipBrowserSource, true);
        }

        // Turn the hype video ON at the exact same time
        CPH.SlobsSetSourceVisibility(sceneName, raidVideoSource, true);
        ...
        CPH.SlobsSetSourceVisibility(sceneName, raidVideoSource, false);
        ...
        if (!string.IsNullOrEmpty(clipId))
        {
            ...
            CPH.SlobsSetSourceVisibility(sceneName, clipBrowserSource, false);
            CPH.SlobsSetBrowserSource(sceneName, clipBrowserSource, "about:blank");
        }
